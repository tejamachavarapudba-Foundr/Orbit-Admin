...
export default async function ProjectsPage({ searchParams }: ProjectsPageProps) {
  ...
  return (
    <>
      <PageHeader title="Startups" description={`${result.meta.totalItems} projects listed on Orbit.`} icon={Rocket} />
      ...
      <div className="p-8">
        <div className="glass overflow-hidden rounded-2xl">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {result.data.map((project) => (
                <tr key={project.id} className="border-b border-border/60 transition last:border-0 hover:bg-white/30">
                  ...
                  <td className="px-5 py-3.5 capitalize text-muted">{project.stage}</td>
                  <td className="px-5 py-3.5">
                    {project.isVerified ? (
          ...
          {result.data.length === 0 ? <p className="p-10 text-center text-sm text-muted">No startups found.</p> : null}
        </div>
