...
const ACCESS_COOKIE = "orbit_admin_token";
const REFRESH_COOKIE = "orbit_admin_refresh";
 ...
 * every authenticated call) the JWT payload just to read role/exp for UI gating. */
...
export const getSession = async () => {
  ...
  return { token, userId: payload.sub, email: payload.email, role: payload.role, expired: payload.exp * 1000 < Date.now() };
};
