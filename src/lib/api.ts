...
type ApiFetchOptions = {
  ...
  body?: unknown;
  /** Skip the automatic redirect-to-login on unrecoverable auth failure — used by the login form itself. */
  allowAnonymous?: boolean;
};

/** Server-only fetch wrapper: attaches the admin's access token, transparently
 * refreshes it on a 401 and retries once, and redirects to /login if the
 * session can't be recovered. Never called from Client Components — the
 * tokens live in httpOnly cookies and are never exposed to the browser. */
